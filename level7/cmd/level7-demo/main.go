...
import (
	...
	"github.com/ronaldpetty/pn-challenge/level7/internal/app"
)
...
func run(args []string) error {
	if len(args) < 2 {
		return fmt.Errorf("usage: %s <artifacts|index|registry|private-facts|revocation-authority|crdt-update-bus|key-rotator|mcp|rotator|consumer|swimlane|health>", args[0])
	}
	...
	case "key-rotator":
		fs := flag.NewFlagSet("key-rotator", flag.ExitOnError)
		artifactsDir := fs.String("artifacts", "/artifacts", "artifact directory")
		logsDir := fs.String("logs", "/logs", "audit log directory")
		if err := fs.Parse(args[2:]); err != nil {
			return err
		}
		return app.RunKeyRotator(*artifactsDir, *logsDir)
	case "mcp":
